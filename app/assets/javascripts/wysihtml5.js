...
wysihtml5.dom.parse = (function() {
  ...
  function _handleAttributes(oldNode, newNode, rule) {
    ...
    for (; i<classesLength; i++) {
      ...
      for (name in allowedClasses) {
        if (name == currentClass || currentClass.match(name)) {
    ...
    if (oldNode.getAttribute("style")) {

        attributes["style"] = oldNode.getAttribute("style");
    }
...
/**
  AVICUS
 * document.execCommand("insertTag") will create a user tag based on the supplied username.
 */
(function(wysihtml5) {
  wysihtml5.commands.insertTag = {
    exec: function(composer, command, value) {
      return composer.commands.exec("insertHTML", "{tag=" + value + "}");
    },
